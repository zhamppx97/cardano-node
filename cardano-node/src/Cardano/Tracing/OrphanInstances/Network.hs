...
instance HasSeverityAnnotation (ServerTrace addr) where
  getSeverityAnnotation ev =
    case ev of
      ...
      Server.TrEstablishedMiniProtocolTerminated {}     -> Debug
...
instance (Show addr, Show versionNumber, Show agreedOptions, ToObject addr)
  ...
  toObject verb ev =
    case ev of
      ...
      TrReusedIdleConnection remoteAddress ->
        mkObject
          [ "kind" .= String "ReusedIdleConnection"
          , "remoteAddress" .= toObject verb remoteAddress
          ]
      ...
      TrConnectionTerminated connId reason mInState ->
        mkObject $
